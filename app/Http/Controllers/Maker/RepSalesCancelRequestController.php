...
class RepSalesCancelRequestController extends Controller
{
                               ...
                               )
    {
      ...
      $this->module_title            = "Reps/Sales Cancelled Orders Requests";
      $this->module_view_folder      = 'maker.rep_sales_cancel_req_orders';
      $this->maker_panel_slug        = config('app.project.maker_panel_slug');
      $this->module_url_path         = url($this->maker_panel_slug.'/cancel_order_requests');
    }
    ...
    public function index(Request $request)
    {
        ...
        $this->arr_view_data['page_title']       = 'Reps/Sales Cancelled Orders Requests';
    ...
    $sales_man_lead_obj = DB::table($representative_leads)
        ...
        if(isset($arr_search_column['q_ship_status']) && $arr_search_column['q_ship_status']!="")
        {
            ...
            $obj_qutoes  = $obj_qutoes->where($representative_leads.'.ship_status','LIKE', '%'.$search_term.'%');


            $sales_man_lead_obj = $sales_man_lead_obj->where($representative_leads.'.ship_status','LIKE', '%'.$search_term.'%');
        }
    ...
    public function order_confirmation(Request $request)
    {
        ...
        $result = $this->BaseModel->where('id',$form_data['order_id'])->update($updated_arr);


        $leads_details =   $this->BaseModel
        ...
        if($result)
        {
            ...
            {
                     if(isset($leads['maker_confirmation']) && $leads['maker_confirmation'] == 1)
                      {
                            /*Update inventory*/
                            foreach ($leads['leads_details'] as $key => $value) 
                            {
                               $update_qunty = $this->ProductInventoryModel->where('sku_no',$value['sku'])->increment('quantity',$value['qty']);
                            }
                      }

                      $retailer_msg ='Cancel order request approved by '.$leads['maker_data']['first_name'].' '.$leads['maker_data']['last_name'].' for Order No: '.$leads['order_no'];

                      $msg = 'Order '.$leads['order_no'].' has been cancelled by vendor '.$leads['maker_data']['first_name'].' '.$leads['maker_data']['last_name'];
                      ...
                      $notification_arr['to_user_id']   = $leads['maker_id'];
                      ...
                      if(isset($leads['representative_id']) && $leads['representative_id']!='')
                      {
                         $id   = $leads['representative_id'];
                         $type = 'representative';

                          $view_href   = url('/').'/representative/rep_sales_cancel_orders/view/'.base64_encode($order_id);

                      }
                      elseif(isset($leads['sales_manager_id']) && $leads['sales_manager_id']!='')
                      {
                         $id   = $leads['sales_manager_id'];
                         $type = 'sales_manager';
                      ...
                      $maker_id = isset($leads['maker_id'])?$leads['maker_id']:'';

                      $maker_mail = $this->get_email($maker_id);

                      $service_response = $this->GeneralService->rep_sales_cancel_order_mail($leads,$maker_mail);

                      //$service_response = $this->GeneralService->rep_sales_cancel_order_status_mail($form_data['status'],$leads,false,$maker_mail); 
                      ...
                      //$service_response = $this->GeneralService->rep_sales_cancel_order_mail($leads,$admin_email);   

                      $service_response = $this->GeneralService->rep_sales_cancel_order_status_mail($form_data['status'],$leads,false,$admin_email); 
                      ...
                      //$service_response = $this->GeneralService->rep_sales_cancel_order_mail($leads,$email);

                      $service_response = $this->GeneralService->rep_sales_cancel_order_status_mail($form_data['status'],$leads,false,$email);   


                      /*send cancel order mail to retailer*/

                      $retailer_id = isset($leads['retailer_id'])?$leads['retailer_id']:'';

                      $retailer_email = $this->get_email($retailer_id);

                      //$service_response = $this->GeneralService->rep_sales_cancel_order_mail($leads,$retailer_email);
                      $service_response = $this->GeneralService->rep_sales_cancel_order_status_mail($form_data['status'],$leads,false,$retailer_email);
            ...
            else if($form_data['status'] == 0)
            {
                ...
                $retailer_msg ='Cancel order request has been rejected by '.$leads['maker_data']['first_name'].' '.$leads['maker_data']['last_name'].' for Order No: '.$leads['order_no'];
                ...
                $this->GeneralService->rep_sales_cancel_order_status_mail($form_data['status'],$leads,$reason,$retailer_email);


                $retailer_view_href = url('/retailer/my_orders/order_summary/'.base64_encode($leads['order_no']).'/'.base64_encode($leads['maker_id']));

            }


            /*send notifcation to the retailer*/

              

            $notification_arr                 = [];
            $notification_arr['from_user_id'] = $loggedInUserId;
            //$notification_arr['to_user_id']   = $leads['retailer_id'] or '';
            $notification_arr['to_user_id']   = $retailer_id;
            $notification_arr['description']  = $retailer_msg;
            $notification_arr['title']        = 'Order Request Confirmation';
            ...
            $this->GeneralService->save_notification($notification_arr);
          	...
          	if($form_data['status'] == 2)
          	{
         	        ...
         	        $response['link']        = url('/vendor/rep_sales_cancel_orders');
          	...
          	elseif($form_data['status'] == 0)
          	{
         	        ...
         	        $response['link']        = url('/vendor/representative_orders');
         	        return response()->json($response);
          	}
