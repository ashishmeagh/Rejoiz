...
class RequestFourthSubCategoryController extends Controller
{
    	                       ...
    	                       )
    {
        ...
        $this->BaseModel                       = $this->FourthSubCategoryModel;
        ...
        $this->module_view_folder              = "maker.request_fourth_level_category";
        $this->module_title                    = "Request Fourth Level Categories";
        $this->module_url_path                 = url(config('app.project.maker_panel_slug').'/request_fourth_level_category');      
    ...
    function get_category_details(Request $request)
    {
        ...
        $obj_subcategory = DB::table($fourth_sub_category_details)
                                ...
                                ->where($prefixed_fourth_sub_category.'.maker_id','=',$loggedInUserId)
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = $this->FourthSubCategoryModel
                                                    // ->where('third_sub_category_id',$request->input('third_sub_category'))
                                                    ->whereHas('translations',function($query) use($request)
                                                    {
                                                        $query->where('locale','en')
                                                            ->where('fourth_sub_category_name',$request->input('fourth_sub_category_name_en'));
                                                    });
        ...
        $notif_sub_category_name = $request->input('fourth_sub_category_name_en');
        $user_name = "";
        $user = Sentinel::check();
        if(isset($user))
        {
        ...
        if($result)
        {
            ...
            if(sizeof($arr_lang) > 0 )
            {
               ...
               /*----------------------------------------------------------------------*/
                ...
                if($is_update == false)
                {
                    if($entity->id)
                    {
                        $response['link'] = url('/vendor/request_fourth_level_category');
                    }
                }
                else
                {
            ...
            $notif_category_name = $request->input('fourth_sub_category_name_en');
            ...
            $view_href = url('/').'/admin/fourth_sub_category_name_en/view/'.base64_encode($last_inserted_id);
            ...
            $notification_arr['title']        = 'Request Fourth Level Category';
    ...
    try
    {
        ...
        {  
            ...
            $message = $this->module_title.' has been activated.';
        ...
        else if($multi_action == "delete")
        {
            ...
            $message = $this->module_title.' has been deleted.';
            $type = "success";
            if($update)
            {
                Flash::$type($message);
