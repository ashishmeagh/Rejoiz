...
class CustomerCancelOrderController extends Controller
{
    							...
    							)
    {
    	...
    	$this->arr_view_data           = [];
    	$this->module_title            = "Customer Canceled Orders";
    	$this->module_view_folder      = 'maker.customer_cancel_orders';
        $this->maker_panel_slug        = config('app.project.maker_panel_slug');
        $this->module_url_path         = url($this->maker_panel_slug.'/customer_cancel_orders');
    }
    ...
    public function index(Request $request)
    {
        ...
        $this->arr_view_data['page_title']       = 'Customer Canceled Orders';
    ...
    { 
        ...
        if(isset($arr_search_column['q_refund_status']) && $arr_search_column['q_refund_status']!="")
        {
            ...
            $obj_qutoes = $obj_qutoes->where($prefixed_customer_quotes_tbl.'.refund_status','LIKE', '%'.$search_term.'%');
        }   
        ...
        {  
            ...
            if($search_term == 1)   
            {
                ...
                // Condition added by Harshada On date 29 Aug 2020
        ...
        $obj_qutoes   = $obj_qutoes->groupBy('order_no');   //Order Dublication Check       
        //echo $obj_qutoes->toSql();die;
    ...
    public function get_export_customer_cancel_order(Request $request)
    {
        $loggedInUserId = 0;
        $user = Sentinel::check();
        ...
        return Excel::create('Customer Cancel Orders', function($excel) use ($data) {
        
        $excel->sheet('Customer Cancel Orders', function($sheet) use ($data)
        {
        ...
        // dd($order_id,$getOrderDetails['order_no']);
            ...
            if ($transactionDetails) {
                ...
                if ($refundDetails['status'] == 'succeeded') 
                {
                    ...
                    $response['msg']    = 'Refund is initiated it will reflect to customers account within 5 to 10 business days.';
                    return $response;
