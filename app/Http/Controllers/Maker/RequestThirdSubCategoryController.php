...
class RequestThirdSubCategoryController extends Controller
{
    	                       ...
    	                       )
    {
      ...
      $this->module_view_folder              = "maker.request_third_level_category";
      $this->module_title                    = "Request Third Level Categories";
      $this->module_url_path                 = url(config('app.project.maker_panel_slug').'/request_third_level_category');      
    ...
    function get_category_details(Request $request)
    {
        ...
        $obj_subcategory = DB::table($third_sub_category_details)
                            ...
                            ->where($prefixed_third_sub_category.'.maker_id','=',$loggedInUserId)
                            ->orderBy('id','DESC');
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = $this->BaseModel
                                        // ->where('category_id',$request->input('first_level_category'))
        ...
        $notif_sub_category_name = $request->input('third_level_category_name_en');
        $user_name = "";
        $user = Sentinel::check();
        if(isset($user))
        {
        ...
        if($result)
        {
            ...
            if(sizeof($arr_lang) > 0 )
            {
               ...
               /*----------------------------------------------------------------------*/
                ...
                if($is_update == false)
                {
                    if($entity->id)
                    {
                        $response['link'] = url('/vendor/request_third_level_category');
                    }
                }
                else
                {
            ...
            $notif_category_name = $request->input('third_level_category_name_en');
            ...
            $view_href = url('/').'/admin/request_third_level_category/view/'.base64_encode($last_inserted_id);
            ...
            $notification_arr['title']        = 'Request Third Level Category';
