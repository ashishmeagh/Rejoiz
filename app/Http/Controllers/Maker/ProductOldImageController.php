...
class ProductOldImageController extends Controller {
	...
	public function __construct(CatalogImageModel $CatalogImageModel,
		...
		$this->module_title 						= "Upload Multiple Images";
		$this->module_view_folder 					= 'maker.product_old_images';
					...
					if(isset($exp_array[5])){	
						...
						if(isset($exp_array[5])){
							...
							if(isset($exp_array[4])){
									...
									$cb_products_multiple_image = DB::table('product_multiple_images')
													->leftJoin('products', 'products.id', '=', 'product_multiple_images.product_id')
													->where('products.user_id', $vendor_id)
													->where('product_multiple_images.sku', $exp_array[4])
													->where('product_multiple_images.is_deleted', '=', 0)													
													->select(['product_multiple_images.sku','product_multiple_images.product_id'])
													->get()->toArray();


									//	dump(count($cb_products_multiple_image));
									if(empty($cb_products_multiple_image) || count($cb_products_multiple_image) < 5)
									{
										//dump('in if',count($cb_products_multiple_image));
														...
														if (array_key_exists($arr_sku_name,$arr_check_cnt))
														  {
														  		...
														  		if(count($cb_products_multiple_image) < 5)
																{
														  		if($arr_img_cnt < 6){
