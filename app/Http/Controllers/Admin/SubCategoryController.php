...
class SubCategoryController extends Controller
{
    	                       ...
    	                       )
    {
      ...
      $this->module_view_folder              =   "admin.sub_category";
      $this->module_title                    =   "Sub Categories";
      $this->module_url_path                 =   url(config('app.project.admin_panel_slug').'/sub_category');      
    ...
    function get_category_details(Request $request)
    {
        ...
        $obj_subcategory = DB::table($sub_category_details)
                                ...
                                ->where($sub_category_details.'.admin_confirm_status','=','0')
                                ->orderBy('id','DESC');
        /* ---------------- Filtering Logic ----------------------------------*/                    
        ...
        if(isset($arr_search_column['q_status']) && $arr_search_column['q_status']!="")
        {
            ...
            $obj_subcategory = $obj_subcategory->where($sub_category_details.'.is_active',$search_term);
        }
                        ...
                        {   if($is_active == '0')
                            {
                                ...
                                if (count($product_details) > 0) {
                                    ...
                                    $msg = "Sub Category contains active product(s), can not change sub category status";
    ...
    try
    {
        ...
        {   
           $message = "Subcategory contains active product(s) can not change status";
    ...
    public function perform_delete($id)
    {
        ...
        $cat_count_in_sub_cat = $this->ThirdSubCategoryModel->where('sub_category_id',$id)->count();
    ...
    try
    {
        ...
        if (count($product_details) == 0) 
        {
            ...
            array_push($deactive_id_arr, $sub_cat_arr[0]['subcategory_name']);
        }
        else
        {   
           $message = "Subcategory contains active product(s) can not change status";
           array_push($active_id_arr, $sub_cat_arr[0]['subcategory_name']);
    ...
    catch(\Exception $e)
        {
            ...
            {   $type = "error";
                $message = "Cannot perform this action Sub categories contains active products";
                if(count($active_id_arr)==1)
                {
                    $message = "Cannot perform action Sub category contains active products";
                }
            }
            ...
            if(count($active_id_arr)>0 && count($deactive_id_arr)>0)
            {
                ...
                $active_message  = implode(',', $active_id_arr);
                $deactive_message = implode(',', $deactive_id_arr);
                $message = "List of deactivated Sub categories: ".$deactive_message.",and some sub categories can not be deactivated because they contains active products, following is list of active sub categories: ".$active_message;
            }
        ...
        {  
            ...
            $message = "Sub categories activated successfully";
            $type = "success";
            if($update)
            {
                Flash::$type($message);
    ...
    public function perform_activate($id_arr)
    {
        ...
        {  
            ...
            $update = $this->SubCategoryModel->where('id',$category_id)->update(['is_active'=>'1']);
            //dd($update);
        }
        return $update;
    }
    ...
    public function perform_multi_delete($id_arr)
    {
        ...
        {  
            $category_id = isset($record_id)?base64_decode($record_id):0;
            $cat_count_in_sub_cat = $this->ThirdSubCategoryModel->where('sub_category_id',$category_id)->count();
            ...
            if($cat_count_in_sub_cat > 0)
            {
               ...
               $delete = $this->SubCategoryModel->where('id',$category_id)->delete();   
            }
