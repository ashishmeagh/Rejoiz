...
class CustomerQuotesController extends Controller
{
    ...
    */
                                ...
                                )
    {
        ...
        $this->module_view_folder      = 'admin.customer_quotes';
...
*/
                                ...
                                ->where($prefixed_customer_quotes_tbl.'.order_cancel_status','!=','1')
        ...
        $json_result  = $json_result->editColumn('created_at',function($data) use ($current_context)
                        ...
                        {   
                          /*  
                            ...
                            //for maker commission status 
                            $vendor_payment = '';

                            if(isset($data->stripe_trxn_status) || $data->stripe_trxn_status!='' || $data->stripe_trxn_status!=null)
                            {
                                $vendor_payment = 'Paid';
                            }
                            else
                        ...
                        {   
                            ...
                            // $ord_retail_price = $ord_retail_price - $data->shipping_charge;
                             ...
                             $vendor_pay_amount = $ord_retail_price - $admin_commission_amount;

                             if($data->ship_status == 1  && $data->is_direct_payment == 0 && $data->payment_term != 'Net30' && $data->is_split_order == 0 && $data->maker_commission_status!=1)
                             {
                                $pay_vendor_button = '<button type="button" class="btn btn-outline btn-info btn-circle show-tooltip"  id="pay_vendor_btn" title="Pay Vendor"  onclick="fillData('.$ord_retail_price.','.$vendor_pay_amount.','.$admin_commission.','.$admin_commission_amount.','.$data->maker_id.','.$data->id.')" style="'.$is_disabled.'" >Pay Vendor</button>';
                             }
...
*/
        ...
        { 
            ...
            if($value->stripe_trxn_status == 3)
            {
              $vendor_payment_status =  'Failed';
            }else if($value->stripe_trxn_status == 2)
            {
            ...
            $arrayResponseData['Customer']              = $customer_name; 
    ...
    public function view($enquiry_id)
    {
        ...
        $enquiry_arr = $split_order_arr = $main_split_order_no = $orderCalculationData = $tracking_details = [];
        ...
        $enquiry_obj = $this->BaseModel->with(['quotes_details.product_details.brand_details',
                                        ...
                                        ->where($prefixed_customer_quotes_tbl.'.id',$enquiry_id)->first();                               
        ...
        if($enquiry_arr_id!=0 && $enquiry_arr_order_no!='')
        {
            ...
            if(isset($tracking_details) && count($tracking_details) > 0){
    ...
    public function download_report()
    {
        ...
        $from_date = date("Y-m-d",strtotime(strval($from_date))).' 00:00:00';
        $to_date   = date("Y-m-d",strtotime(strval($to_date))).' 23:59:59';
        ...
        foreach ($enquiry_arr as $key => $value) {
           ...
           $order_data['Customer Name'] = $value['user_details']['first_name'].' '.$value['user_details']['last_name'];
           $order_data['Company Name']  = $value['maker_data']['company_name'];
           $order_data['Order Date']    = us_date_format($value['created_at']);

           $order_data['Total Price']   = number_format((float)$value['total_retail_price'], 2, '.', '');
