...
class CustomerController extends Controller
{
    ...
    */
    						...
    						)
    {
        ...
        $this->module_url_path    = url(config('app.project.admin_panel_slug')."/customer");    
...
*/                                return $build_status_net_30_btn;
    ...
    {
      $customer_status      = $request->input('customerAprovalStatus');
      ...
      {
        $this->UserModel->where('id',$customer_id)->update(['is_approved'=>1]);
      ...
      {
        $this->UserModel->where('id',$customer_id)->update(['is_approved'=>0]);
    ...
    {         $type  = \Request::segment(4);
        ...
        if($type=="pdf")
        {
         $inventory_pdf =  $this->PdfReportService->downloadPdfCustomer($type);
         
          return $inventory_pdf->download('customer report.pdf');
        }
        else
        {
            $this->ReportService->downloadExcelCustomer($type);
        }
    }
