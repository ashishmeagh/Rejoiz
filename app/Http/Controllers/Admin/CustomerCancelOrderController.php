...
class CustomerCancelOrderController extends Controller
{
    							...
    							)
    {
    	...
    	$this->module_view_folder      = 'admin.customer_cancel_orders';
        $this->admin_panel_slug        = config('app.project.admin_panel_slug');
        $this->module_url_path         = url($this->admin_panel_slug.'/customer_cancel_orders');
        $this->module_url              = url('/');
    }
    ...
    { 
      ...
      $retailer_tbl_name               = $this->CustomerModel->getTable();
      $prefixed_customer_tbl           = DB::getTablePrefix().$this->CustomerModel->getTable(); 
      ...
      $obj_qutoes = DB::table($customer_quotes_tbl_name)
                                ...
                                ->leftjoin($prefixed_customer_tbl,$prefixed_customer_tbl.'.user_id','=',$prefixed_user_tbl.'.id')
                                ...
                                ->leftjoin($maker_tbl_name,$prefixed_maker_tbl.'.user_id','=',$prefixed_customer_quotes_tbl.'.maker_id')
                                 ...
                                 ->leftjoin($prefixed_transaction_mapping_tbl,function($join) use($customer_quotes_tbl_name,$prefixed_transaction_mapping_tbl){
                                    $join->on($customer_quotes_tbl_name.'.id','=',$prefixed_transaction_mapping_tbl.'.order_id')
                                    ->on($customer_quotes_tbl_name.'.order_no','=',$prefixed_transaction_mapping_tbl.'.order_no');
                                    })
        ...
        if(isset($arr_search_column['q_payment_status']) && $arr_search_column['q_payment_status']!="")
        {
           /* $search_term  = $arr_search_column['q_payment_status'];
            ...
            if($search_term == 1)
            {
                //$obj_qutoes = $obj_qutoes->where($prefixed_customer_quotes_tbl.'.ship_status','=',0);
...
// dd($arr_search_column['q_refund_status']);
        if(isset($arr_search_column['q_order_from_date']) && $arr_search_column['q_order_from_date']!="" && isset($arr_search_column['q_order_to_date']) && $arr_search_column['q_order_to_date']!="" && isset($arr_search_column['q_refund_status']) && $arr_search_column['q_refund_status']!="")
        {
            ...
            $obj_qutoes   = $obj_qutoes->whereDate($prefixed_customer_quotes_tbl.'.created_at', '>=', $order_from_date);
            $obj_qutoes   = $obj_qutoes->whereDate($prefixed_customer_quotes_tbl.'.created_at', '<=', $order_to_date);
        ...
        if(isset($arr_search_column['q_refund_status']) && $arr_search_column['q_refund_status'] == 0)
            {
                // dd($arr_search_column['q_refund_status']);
                $obj_qutoes = $obj_qutoes->where($prefixed_customer_quotes_tbl.'.maker_confirmation','1');
            }
      ...
      if(isset($arr_search_column['q_order_to_date']) && $arr_search_column['q_order_to_date']!="" && $arr_search_column['q_order_from_date']=="" && isset($arr_search_column['q_refund_status']) && $arr_search_column['q_refund_status']!="")
         ...
         if(isset($arr_search_column['q_refund_field']) && $arr_search_column['q_refund_field']!="")
        {
            ...
            if ($search_term_refund_status == '0') {
                 ...
                 $obj_qutoes   = $obj_qutoes->where($prefixed_customer_quotes_tbl.'.refund_status', $search_term_refund_status)->where($prefixed_transaction_mapping_tbl.'.transaction_status','LIKE', '2');
    ...
    public function get_export_customer_cancel_orders(Request $request)
    {
      $customer_quotes_tbl_name     = $this->BaseModel->getTable();        
      $prefixed_customer_quotes_tbl = DB::getTablePrefix().$this->BaseModel->getTable();
...
// dd($arr_search_column['q_refund_status']);
    ...
    public function view($enquiry_id)
    {
        ...
        $enquiry_obj = $this->BaseModel->with(['quotes_details.product_details.brand_details','maker_details','maker_data','transaction_mapping','user_details','user_details.customer_details'])
                                                ->where('id',$enquiry_id)->first();                               
        ...
        if($getOrderDetails)
        {
            ...
            if ($transactionDetails) {
                ...
                if ($refundDetails['status'] == 'succeeded') 
                {
                    ...
                    $refundData['received_by']         = $getOrderDetails['customer_id'] or ' ';
                    ...
                    $arr_notify_data['to_user_id']   = $getOrderDetails['customer_id'] or '';

                    $arr_notify_data['description']  = 'For your cancelled order('.$getOrderDetails['order_no'].') refund is initiated, it will be reflected within 5 to 10 business days. Transaction id: '.$refundDetails['balance_transaction'];
