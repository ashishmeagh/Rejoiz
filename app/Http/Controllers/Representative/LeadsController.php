...
class LeadsController extends Controller
{
                           ...
                           )
  {
    ...
    $this->module_view_folder = 'representative.leads';
    $this->representative_panel_slug   = config('app.project.representative_panel_slug');
    $this->module_url_path    = url($this->representative_panel_slug.'/leads');
  ...
  public function reorder(Request $request)
  {
    ...
    $ship_first_name = $arr_order_details['address_details']['ship_first_name'];
    $ship_last_name = $arr_order_details['address_details']['ship_last_name'];
    /*if(isset($arr_order_details['address_details']['ship_first_name'])){
      $exp_ship_first_name = explode($arr_order_details['address_details']['ship_first_name'], " ");
      if(!empty($exp_ship_first_name)){
            $ship_first_name = isset($exp_ship_first_name[0]) ? $exp_ship_first_name[0] : $arr_order_details['address_details']['ship_first_name'];
            $ship_last_name = isset($exp_ship_first_name[1]) ? $exp_ship_first_name[1] : $arr_order_details['address_details']['ship_last_name'];
          }
    }
*/
...
/*        $retailer_arr = $this->RetailerModel->with(['user_details'])
      ...
      /*if(count($is_confirm)<=0)
      { 
        Flash::error('Please add atleast one maker in your list');
              return redirect()->back();
      } */                  
      ...
      $this->arr_view_data['module_title']    = $this->module_title;
      $this->arr_view_data['page_title']      = 'Retailer Details';
      $this->arr_view_data['module_url_path'] = $this->module_url_path;
  ...
  public function get_order_list($arr_search_column=false,$module_data=false,$representative_id=false,$confirmed_flag=false,$pending_flag=false,$completed_flag=false,$approved_flag=false,$net_30_completed_orders=false,$net_30_pending_orders=false)
  {
      ...
      $lead_obj = DB::table($representative_leads)
                          ...
                          ->leftjoin($prefix_maker_table,$prefix_maker_table.'.user_id','=',$representative_leads.'.maker_id')
                           ...
                           ->where($prefix_representative_leads_tbl.'.representative_id','=',$representative_id)
    ...
    $rep_sales_order_obj = DB::table($representative_leads)
      ...
      if(isset($arr_search_column['q_maker_name']) && $arr_search_column['q_maker_name']!="")
      {
          ...
          $rep_sales_order_obj = $rep_sales_order_obj
                                ->where($prefix_maker_table.'.company_name','LIKE', '%'.$search_term.'%')
                                ->where($representative_leads.'.is_confirm','!=',4);
     ...
     /* if(isset($arr_search_column['q_payment_status']) && $arr_search_column['q_payment_status']!="")
      {  
        ...
        else
        {
        	    ...
        	    $lead_obj = $lead_obj->whereExists(function($query) use ($prefix_transaction_mapping,$representative_leads)
              {
                    $query->select(\DB::raw("
                            transaction_mapping.order_id,
                            transaction_mapping.order_no
              ...
              $rep_sales_order_obj = $rep_sales_order_obj->whereExists(function($query) use ($prefix_transaction_mapping,$representative_leads)
              {
                    $query->select(\DB::raw("
                            transaction_mapping.order_id,
                            transaction_mapping.order_no
  ...
  public function lead_listing(Request $request)
  {
    ...
    $data = $this->get_order_list($search_data,$module_data,$representative_id,$confirmed_flag,$pending_flag,$completed_flag,$approved_flag,$net_30_completed_orders,$net_30_pending_orders);
    ...
    $json_result     = $json_result->editColumn('enc_id',function($data)
                       ...
                       ->editColumn('payment_status',function($data) use ($current_context)
                        ...
                        {   
                            ...
                            //if order is save means is_confirm is 4 then show all vendor product
                        ...
                        ->editColumn('build_action_btn',function($data) use ($module_data)
                        {
                            ...
                            $build_edit_action = $build_view_action = $build_reorder_action = $build_chat_action = '';
                            ...
                            {
                              $build_edit_action = '<a href="'.$module_data['module_url'].'/find_products/'.base64_encode($data->order_no).'/edit"  data-size="small" title="Edit Order Details" class="btn btn-circle btn-success btn-outline show-tooltip">Edit</i></a>';
                            }
                        ...
                        ->editColumn('created_at',function($data)
                         ...
                         ->editColumn('comission_status',function($data)
                          {
                            /*$comission_status = $data->rep_commission_status;
  ...
  { 
       ...
       //                              ->join($vendor_rep_mapping_table,$vendor_rep_mapping_table.'.vendor_id','=',$prefixed_product_tbl.'.user_id','left outer')
       //                              // ->leftjoin($prefixed_maker_tbl,$maker_tbl.'.user_id','=',$vendor_rep_mapping_table.'.vendor_id')
       //                              ->where($user_table.'.status','1')
       //                              ->where($user_table.'.is_approved','1')
       //                              ->where($product_tbl_name.'.product_complete_status','4')
       //                              ->where($product_tbl_name.'.is_active','1')
  ...
  { 
            ...
            if(isset($product_arr) && sizeof($product_arr)>0)
            {
              ...
              { 
                ...
                { 
                  ...
                  $new_product_id = base64_encode($first_product_id);
      ...
      $this->arr_view_data['module_base_path'] = url($this->representative_panel_slug);
  ...
  */
  ...
  public function update_product_qty(Request $request)
  {
     ...
     /*----update total wholesale price as per total product dis ,total shipping charges ,tot shiping dis------*/
      /*
      $order_no = $update_product['arr_responce']['order_no'];
  ...
  { 
    ...
    if($response['status'] == 'SUCCESS')
    {
        ...
        DB::commit();
        $response['status']      = 'SUCCESS';               
        $response['description'] = 'Retailer details saved.';
  ...
  public function view_lead_listing($enc_lead_id,$order_no)
  {

    $loggedInUserId = 0;
    $user = Sentinel::check();
    $main_split_order_no = $split_order_arr = [];
  ...
  public function update_lead_listing($lead_id)
  {
    ...
    $leads_arr = [];
    $obj_data = $this->RepresentativeLeadsModel
                     ->with(['leads_details.product_details','address_details','retailer_user_details'=>function($q1)
                     {
    ...
    {   
      ...
      if($is_update)
      {
          ...
          /******************Notification to maker START*******************************/
              ...
              $arr_event['description']  = 'Lead confirmed by a'.$first_name.' '.$last_name.' .';
              $arr_event['title']        = 'Lead confirmed by a representative';
      ...
      {
        $response['status']      = "error";
        $response['description'] = "Error occurred while confirming lead.";
        $response['url'] = $this->module_url_path;
        return response()->json($response); 
      }
    }
  ...
  {  
    ...
    /*---------------------------------------*/
      ...
      $total_wholesale_price = $total_product_discount = $total_shipping_charges = 
                $total_shipping_charge = $total_shipping_discount = $total_wholsale_price = $mkId = 0; 
         ...
         if(isset($order_data['order_details']) && count($order_data['order_details'])>0)
         {
            ...
            foreach($order_data['order_details'] as $key => $orders)
            {
              $updated_arr['wholesale_price'] = $updated_arr['unit_wholsale_price'] = $updated_arr['product_shipping_charge'] = $updated_arr['shipping_charges_discount'] = $updated_arr['shipping_charges'] = $updated_arr['product_discount'] = 0;
                ...
                $updated_arr['shipping_charges'] = $updated_arr['product_shipping_charge'] = 0;
  ...
  public function finalize_lead(Request $request,$enc_order_no = false)
  {
    ...
    if(isset($order_no))
    {
      ...
      if($type=='confirm_requested')
      {
        ...
        $msg = "Order has been confirmed and sent to retailer for approval.";
      }
  ...
  public function get_bag_data(Request $request)
  {
    $user = Sentinel::check();
    $arr_final_data = [];
  ...
  /************************Notification Event START**************************/
    ...
    {
      $this->arr_view_data['module_title']    = $this->module_title;
      $this->arr_view_data['page_title']      = 'Retailer Orders';
      $this->arr_view_data['module_url_path'] = $this->module_url_path;

      return view($this->module_view_folder.'.retailer',$this->arr_view_data);
    }


    public function get_retailer_order_list($arr_search_column=false,$module_data=false,$representative_id=false,$arrsortedRetailerIdByZipCode=false)
    {
      $user_table        =  $this->UserModel->getTable();
      $prefix_user_table = DB::getTablePrefix().$user_table;
      ...
      $lead_obj = DB::table($representative_leads)
                          ...
                          $lead_obj = $lead_obj->whereIn($prefix_representative_leads_tbl.'.retailer_id',$arrsortedRetailerIdByZipCode);    

                          if(isset($representative_id) && $representative_id!='')
                           {
                              $lead_obj = $lead_obj->where($prefix_representative_leads_tbl.'.representative_id','=',$representative_id);
                           }
       //dd(1221212,$lead_obj->get()->toArray());                    
      /* ---------------- Filtering Logic ----------------------------------*/  
     ...
     if(isset($arr_search_column['q_order_no']) && $arr_search_column['q_order_no']!="")
      ...
      if(isset($arr_search_column['q_lead_status']) && $arr_search_column['q_lead_status']!="")
      {
          ...
          if ($search_term == "2") {
            ...
            $lead_obj = $lead_obj->where($prefix_representative_leads_tbl.'.is_confirm','=', $search_term)->orwhere($prefix_representative_leads_tbl.'.is_confirm','=', 0);
          }
          else{
  ...
  public function lead_retailer_listing(Request $request)
  {
    $search_data = $request->input('column_filter');
    ...
    $arrRep     = [];
    $arrZipCode = [];
    $objRep = $this->RepresentativeModel->where('user_id',$representative_id)->first(['area_id']);
    if($objRep)
    {
    ...
    //$arrZipCode = array_column($arrZipCode,'zip_code');

    //Match zip code retailer
      
    $arrZipCodeRetailer = $this->RetailerModel->whereIn('billing_zip_postal_code',$arrZipCode)->get(['user_id'])->toArray();

    $arrZipCodeRetailer = array_column($arrZipCodeRetailer,'user_id');
    
    $data = $this->get_retailer_order_list($search_data,$module_data,$representative_id,$arrZipCodeRetailer);
