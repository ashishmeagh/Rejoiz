...
class RepSalesCanceledOrderController extends Controller
{
    							...
    							)
    {
    	...
    	$this->module_view_folder      = 'representative.rep_sales_cancel_orders';
        $this->representative_panel_slug     = config('app.project.representative_panel_slug');
        $this->module_url_path         = url($this->representative_panel_slug.'/rep_sales_cancel_orders');
    }
    ...
    { 
        ...
        $obj_qutoes = DB::table($leads_table)
                            ...
                            ->leftjoin($prefixed_transaction_mapping_tbl,function($join) use($prefix_representative_leads_tbl,$prefixed_transaction_mapping_tbl){
                              ...
                              ->on($prefix_representative_leads_tbl.'.order_no','=',$prefixed_transaction_mapping_tbl.'.order_no');
                                ...
                                ->where($prefixed_leads_table.'.representative_id',$loggedInUserId)

                                ->orderBy($prefixed_leads_table.".id",'DESC');
        

        if(isset($arr_search_column['representative_id']) && $arr_search_column['representative_id']!="" && $arr_search_column['representative_id']!='0')
        {

            $retailer_id = base64_decode($arr_search_column['representative_id']);
            
            $obj_qutoes = $obj_qutoes->where($prefixed_leads_table.'.representative_id',$retailer_id)->where($prefixed_leads_table.'.representative_id',$loggedInUserId);
        }
        else
        {
            $obj_qutoes = $obj_qutoes->where($prefixed_leads_table.'.representative_id',$loggedInUserId);   
        } 
        ...
        if(isset($arr_search_column['q_total_wholesale_cost']) && $arr_search_column['q_total_wholesale_cost']!="")
        {
            ...
            $obj_qutoes = $obj_qutoes->having('total_wholesale_price','LIKE', '%'.$search_term.'%');
        }   
                ...
                $obj_qutoes = $obj_qutoes->whereExists(function($query){
                    $query->select(\DB::raw("
                      ...
                      FROM
                          `transaction_mapping`
                      WHERE
                          `transaction_mapping`.`order_no` = representative_leads.order_no AND `transaction_mapping`.`order_id` = representative_leads.id  AND
                                `transaction_mapping`.transaction_status = 3
                    "));
                });
            }
            if($search_term == 2){
                $obj_qutoes = $obj_qutoes->whereExists(function($query){
                    $query->select(\DB::raw("
                      ...
                      WHERE
                          `transaction_mapping`.`order_no` = representative_leads.order_no AND `transaction_mapping`.`order_id` = representative_leads.id AND
                                `transaction_mapping`.transaction_status = 2
                    "));
                });
            }
            }
    ...
    public function view(Request $request, $enquiry_id = 0)
    {
       ...
       $enquiry_obj = $this->BaseModel->with(['transaction_mapping',
    	...
    	if($enquiry_obj)
    	{
    		...
    		if($enquiry_arr['representative_id'] != $loggedInUserId)
			{
