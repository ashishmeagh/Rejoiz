         ...
         <div class="col-lg-12">
            ...
            <div class="white-box">   
              ...
              <div class="col-sm-12 text-right export-btn-div p-0 mb-3">
                <button type="button" onclick="orderByRepSalesExport()" title="Export order by rep/sales as .csv" class="btn btn-success waves-effect waves-light filter_btn" value="export" id="orderByRepSalesExport">Export</button>
              </div>       
                  ...
                  <form class="form-horizontal" id="report_form">
                    <div class="form-group row retailer_data_filter_row">
                     ...
                     <div class="col-sm-2 retailer_data_filter_btn">
                        ...
                        <a href="{{$module_url_path}}/order_from_representative" class="btn btn-outline btn-info btn-circle show-tooltip space-btn-cirlce" title="Refresh"><i class="fa fa-refresh"></i> </a>
                     </div>
              </div>
            </form>
...
<script type="text/javascript">
  ...
  $(document).ready(function()
  {
      ...
      var net_30_completed_flag = '{{$net_30_completed_flag or 0}}'; 

      var net_30_pending_flag = '{{$net_30_pending_flag or 0}}'; 
      ...
      'data': function(d)
        {
          ...
          d['net_30_completed_flag']           = net_30_completed_flag;
          d['net_30_pending_flag']             = net_30_pending_flag;
   ...
   columns: [
      
      {
        ...
        { 
            if (row.is_split_order == 1)
            {
                ...
                { 
                    return `<a href="`+module_url_path+`/order_summary/`+btoa(row.order_no)+`/`+btoa(row.maker_id)+`">`+row.order_no+`</a><br> <span class="label label-success">Split</span>`+`</a>`+` <br><br><span class="label label-success">Request Rejected</span>`;          
                }
                else
                {
                    return `<a href="`+module_url_path+`/order_summary/`+btoa(row.order_no)+`/`+btoa(row.maker_id)+`">`+row.order_no+`</a><br> <span class="label label-success">Split</span>`
            ...
            if(row.order_cancel_rejected_status=='1')
            {
               return `<a href="`+module_url_path+`/order_summary/`+btoa(row.order_no)+`/`+btoa(row.maker_id)+`">`+row.order_no+`</a><br> <span class="label label-success">Request Rejected</span>` 
            } 
      ...
      // {data: 'company_name', "orderable": false, "searchable":false}, 
      {
        render(data, type, row, meta)
        {            
            return `<a href="`+module_url_path+`/order_summary/`+btoa(row.order_no)+`/`+btoa(row.maker_id)+`">`+row.representative_user_name+`</a>`;
        },
        "orderable": false, "searchable":false
      },
      ...
      //{data: 'representative_user_name', "orderable": true, "searchable":false}, 
      {
        render(data, type, row, meta)
        {
            return `<a href="`+module_url_path+`/order_summary/`+btoa(row.order_no)+`/`+btoa(row.maker_id)+`" class="link_v">`+row.company_name+`</a>`;
        },
        "orderable": false, "searchable":false
      },
...
function orderByRepSalesExport(){

        var q_order_no              = $("input[name='q_order_no']").val();
        var q_lead_date             = $("input[name='q_lead_date']").val()
        var q_representative_name   = $("input[name='q_representative_name']").val()
        var q_maker_company         = $("input[name='q_maker_company']").val()
        var q_lead_status           = $("select[name='q_lead_status']").val()
        var q_payment_status        = $("select[name='q_payment_status']").val()
        var q_ship_status           = $("select[name='q_ship_status']").val()
        var q_total_costing_retail  = $("input[name='q_total_costing_retail']").val()
        var q_total_costing_wholesale  = $("input[name='q_total_costing_wholesale']").val()
        ...
        var my_net_30_completed_flag   = '{{$net_30_completed_flag or 0}}'; 
        var my_net_30_pending_flag     = '{{$net_30_pending_flag or 0}}'; 
      
        var net_30_completed_flag   = '{{$net_30_completed_flag or 0}}'; 
        var net_30_pending_flag     = '{{$net_30_pending_flag or 0}}'; 
        var net_30_completed_flag   = net_30_completed_flag;
        var net_30_pending_flag     = net_30_pending_flag;

        var csv_name = 'order_by_reps_sales.csv';
        if(net_30_completed_flag == '1'){
           csv_name = 'net30_complete_order_by_reps_sales.csv';
        } else if(net_30_pending_flag == '1'){
            csv_name = 'net30_pending_order_by_reps_sales.csv';
        }

        $.ajax({
          url: module_url_path+'/get_export_order_by_representative',
          data: {q_order_no:q_order_no,q_lead_date:q_lead_date,q_representative_name:q_representative_name,q_maker_company:q_maker_company,q_lead_status:q_lead_status,q_payment_status:q_payment_status,q_ship_status:q_ship_status,q_total_costing_retail:q_total_costing_retail,q_total_costing_wholesale:q_total_costing_wholesale,q_from_date:q_from_date,q_to_date:q_to_date,q_order_rep_sales_name:q_order_rep_sales_name,pending_flag:pending_flag,completed_flag:completed_flag,net_30_completed_flag : net_30_completed_flag,net_30_pending_flag : net_30_pending_flag,my_net_30_completed_flag : my_net_30_completed_flag,my_net_30_pending_flag:my_net_30_pending_flag},
