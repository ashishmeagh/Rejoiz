...
<!-- Page Content -->
<div id="page-wrapper">
    <div class="container-fluid manage_brand_page">
        ...
        <div class="col-sm-12">
            <div class="white-box">
                {!! Form::open([ 'url' => $module_url_path.'/multi_action',
                    ...
                    {{ csrf_field() }}
                    <div class="onerowflex">
                    <a href="{{ url($module_url_path.'/create') }}" class="btn btn-outline btn-info show-tooltip add-brandbtn" title="Add More"> Add a Brand</a> 
                    <div class="top_small_icon 4tbn m-b-0">
...
<script type="text/javascript">
  ...
  $(document).ready(function() 
  {
      ...
      stateSave: true,
            ...
            ajax: {
                    url:module_url_path+'/all_brands',

                   'data': function(d)
                        {
                          d['column_filter[q_brand_name]'] = $("input[name='q_brand_name']").val()
                          d['column_filter[q_brand_status]'] = $("select[name='q_brand_status']").val()
            ...
            columns: [
                                  
                {
...

                  {
                     render : function(data, type, row, meta) 
                     {
                        return '<img class="zoom-img" '+  
                             ' src="'+img_path+'/'+row.brand_image+'" height="100px" width="100px"/>';
                     },
                     "orderable": false,
                     "searchable":false
                  },
                 ...
                 {data: 'is_active',
                    ...
                    {   
                        if(row.is_active == 1)
                        {
                            return `<input type="checkbox" checked data-size="small"  data-enc_id="`+btoa(row.id)+`" id="status_'.$data->id.'"  class="js-switch toggleSwitch" data-type="deactivate" data-color="#99d683" data-secondary-color="#f96262" action="deactivate" onchange="change_status(this)"/>`
                        }
                        else
                        {
                            return `<input type="checkbox" data-size="small" data-enc_id="`+btoa(row.id)+`"  class="js-switch toggleSwitch" data-type="activate" data-color="#99d683" data-secondary-color="#f96262" onchange="change_status(this)" action="activate"/>`
                        }
                    }
                  },
        ...
        $("#table_module").find("thead").append(`<tr>
                   ...
                   <td></td>
                    ...
                    <td><input type="text" name="q_brand_name" placeholder="Search" class="search-block-new-table column_filter form-control" /></td>

                    <td>
                  <select name="q_brand_status" class="search-block-new-table column_filter form-control td-select-dropdown" id="q_brand_status" onchange="filterData();">
    ...
    function change_status(reference)
    {
      ...
      if(type == 'deactivate')
      {
          msg = 'Are you sure? Do you want to deactivate this brand.';
          action_attribute = "activate";
      }
      else if(type == 'activate')
      {
         msg = 'Are you sure? Do you want to activate this brand.';
         ...
         function(isConfirm){
          ...
          if (isConfirm) 
          {
           $.ajax({
               ...
               type: 'GET',
               dataType:'json',
               data:{brand_id:enc_id,status:status},
               beforeSend: function() 
              {
  ...
  {
    confirm_action(ref,event,'Are you sure? Do you want to delete this brand.');
  }
