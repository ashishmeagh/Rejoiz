...
.table.table-striped tr td .btn-circle{
   padding: 4px 10px;
...
<script type="text/javascript">
  ...
  $(document).ready(function() 
  {
      ...
      stateSave: true,
            ...
            ajax: {
                    url:module_url_path+'/get_maker_promotions',

                   'data': function(d)
                        {
                          // d['column_filter[q_promotion_type]'] = $("input[name='q_promotion_type']").val()
                          d['column_filter[q_title]']          = $("input[name='q_title']").val()
                          d['column_filter[q_from_date]']      = $("input[name='q_from_date']").val()
                          d['column_filter[q_to_date]']        = $("input[name='q_to_date']").val()
            ...
            columns: [
                  ...
                  // {data: 'promotion_type_name',"orderable": true, "searchable":false},
                  {data: 'title',"orderable": true, "searchable":false},
                  {data: 'from_date',"orderable": true, "searchable":false},
                  {data: 'to_date',"orderable": true, "searchable":false},
                  {data: 'promo_code',"orderable": true, "searchable":false},
                  // {data: 'discount',"orderable": true, "searchable":false},
                  ...
                  {data: 'is_active',
                    ...
                    {   
                        if(row.is_active == 1)
                        {
                            return `<input type="checkbox" checked data-size="small"  data-enc_id="`+row.id+`" id="status_'.$data->id.'"  class="js-switch toggleSwitch" data-type="deactivate" action="deactivate" data-color="#99d683" data-secondary-color="#f96262" onchange="change_status(this)"/>`
                        }
                        else
                        {
                            return `<input type="checkbox" data-size="small" data-enc_id="`+row.id+`"  class="js-switch toggleSwitch" data-type="activate" action="activate" data-color="#99d683" data-secondary-color="#f96262" onchange="change_status(this)"/>`
                        }
                    }
                  },
        ...
        $("#table_module").find("thead").append(`<tr>   
              ...
              <td></td>       
                 ...
                 <td><input type="text" name="q_title" placeholder="Search" class="search-block-new-table column_filter form-control-small" /></td> 

                <td><input type="text" name="q_from_date" placeholder="Search" onchange="filterData();" class="search-block-new-table column_filter form-control-small datepicker" readonly/></td> 

                  <td><input type="text" name="q_to_date" placeholder="Search" onchange="filterData();" class="search-block-new-table column_filter form-control-small datepicker" readonly/></td> 
                 
                 <td><input type="text" name="q_promo_code" placeholder="Search" onchange="filterData();" class="search-block-new-table column_filter form-control-small"/></td>
                <td>
  ...
  function change_status(reference)
  {
      ...
      var type = $(reference).attr('action');

      if(type == 'deactivate')
      {
          msg = 'Are you sure? Do you want to deactivate this product.';
                ...
                var status = reference.getAttribute('data-type');
                  if (isConfirm) 
                  {
                    $.ajax({
                       ...
                       type: 'GET',
                       dataType:'json',
                       data:{promotions_id:enc_id,status:status},
                       beforeSend: function() 
                       {
  ...
  function confirm_delete(ref,event)
  {
    ...
    confirm_action(ref,event,'Are you sure? Do you want to delete this promotions record.');
  }
