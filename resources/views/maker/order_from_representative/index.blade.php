      ...
      <div class="row bg-title">
         <div class="col-sm-12 top-bg-title">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Orders By Reps / Sales</li>
            </ol>
        </div>
         </div>
         ...
         <div class="col-lg-12">
            <div class="white-box">  
               ...
               <div class="col-sm-12 text-right export-btn-div p-0 mb-3">
                <button type="button" onclick="representativeOrderExport()" title="Export representative orders as .csv" class="btn btn-success waves-effect waves-light filter_btn pull-right" value="export" id="representativeOrderExport">Export</button>  
               </div>         
  ...
  $(document).ready(function()
  {
      ...
      'data': function(d)
        {
          ...
          d['column_filter[q_representative_name]']    = $("select[name='q_representative_name']").val()
      ...
      columns: [
        ...
        render(data, type, row, meta)
        {
            ...
            return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`">`+row.order_no+`</a>`+`<br> <span class="label label-success">Split</span>`;
          }  
            return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`">`+row.order_no+`</a>`;
      ...
      // {data: 'store_name', "orderable": false, "searchable":false}, 
      {
        render(data, type, row, meta)
        {     
          if(row.dummy_store_name == 'null' || row.dummy_store_name == null || row.dummy_store_name == "")     
          {
            //return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`" class="link_v"> - </a>`;
            return `-`;
          }
          else
          {
            /*return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`" class="link_v">`+row.store_name+`</a>`;*/
            return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`" class="link_v">`+row.dummy_store_name+`</a>`;
          }      
                      
        },
        "orderable": false, "searchable":false
      },        
      // {data: 'representative_user_name',"orderable": false,"searchable":false}, 
      {
        render(data, type, row, meta)
        {           
            return `<a href="`+module_url_path+`/view/`+btoa(row.order_no)+`" class="link_v">`+row.representative_user_name+`</a>`;          
        },
        "orderable": false, "searchable":false
      },  
    ...
    /*search box*/
     $("#table_module").find("thead").append(`<tr>
                    ...
                    <td><input type="text" name="q_customer_name" placeholder="Search" class="search-block-new-table column_filter form-control-small" /></td>
                     ...
                     <td>
                       <select class="search-block-new-table column_filter form-control-small" name="q_representative_name" id="q_representative_name" onchange="filterData();">
...
function representativeOrderExport(){
    ...
    $.ajax({
          url: module_url_path+'/get_export_representative_orders',
          data: {q_order_no:q_order_no,q_lead_date:q_lead_date,q_customer_name:q_customer_name,q_representative_name:q_representative_name,q_lead_status:q_lead_status,q_payment_status:q_payment_status,q_ship_status:q_ship_status,q_payment_term:q_payment_term,q_total_costing_retail:q_total_costing_retail,q_total_costing_wholesale:q_total_costing_wholesale,q_order_from_date:q_order_from_date,q_order_to_date:q_order_to_date},
