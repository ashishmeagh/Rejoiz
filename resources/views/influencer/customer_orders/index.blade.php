...
.label.label-success.cancle-btn {
    background-color: #666 !important;
    color: #fff !important; margin-right: 2px; margin-top: 7px;
}
</style>
<!-- Page Content -->
<div id="page-wrapper">
...
<script type="text/javascript">
  ...
  $(document).ready(function()
  {
      ...
      columns: [
      {
        ...
        {
           /* return `<a target="_blank" style="margin-bottom:15px;" href="`+module_url_path+`/view/`+btoa(row.id)+`">`+row.order_no+`</a> <div class="clearfix"></div><span class="label label-success cancle-btn">Split</span><span class="label label-success cancle-btn">Cancled</span>`*/

            if(row.is_split_order == '1')
            {
              return `<a target="_blank" href="`+module_url_path+`/view/`+btoa(row.id)+`">`+row.order_no+`</a> <br><span class="label label-success">Split</span>`
            }
            else if(row.is_split_order == '1' && row.order_cancel_status == '2')
            {
              return `<a target="_blank" style="margin-bottom:15px;" href="`+module_url_path+`/view/`+btoa(row.id)+`">`+row.order_no+`</a> <div class="clearfix"></div><span class="label label-success cancle-btn">Split</span><span class="label label-success cancle-btn">Canceled</span>`              
            }
            else if(row.order_cancel_status == '2')
            {
                return `<a target="_blank" style="margin-bottom:15px;" href="`+module_url_path+`/view/`+btoa(row.id)+`">`+row.order_no+`</a> <div class="clearfix"></div><span class="label label-success cancle-btn">Canceled</span>`
            } 
            else
            {
              return `<a target="_blank" href="`+module_url_path+`/view/`+btoa(row.id)+`">`+row.order_no+`</a>`
            }
      ...
      // {data: 'store_name', "orderable": false, "searchable":false}, 
      {
        render(data, type, row, meta)
        {            
          // target="_blank" href="`+module_url_path+`/view/`+btoa(row.id)+`"
            return `<a javascript:void(0) class="link_v">`+row.customer_name+`</a>`
      ...
      // {data:'company_name',"orderable": false, "searchable":false},
      {
        ...
        {            
          // target="_blank" href="`+module_url_path+`/view/`+btoa(row.id)+`" 
            return `<a javascript:void(0) class="link_v">`+row.company_name+`</a>`
