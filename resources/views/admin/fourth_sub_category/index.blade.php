...
<script type="text/javascript">
  ...
  var table_module = false;
    ...
    $(document).ready(function() 
    {
        ...
        table_module = $('#table_module').DataTable({language: {
            ...
            'processing': '<div class="spinner"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>'
             ...
             ajax: {
                      url: module_url_path+'/get_all_subcategory',
                      data:function(d)
                      {
                         /*send category id */
                        ...
                        d['column_filter[q_third_sub_category]']   = $("input[name='q_third_sub_category']").val()
                        d['column_filter[q_fourth_sub_category]']   = $("input[name='q_fourth_sub_category']").val()
             ...
             columns: [
                  {
                    ...
                    {
                        return '<div class="checkbox checkbox-success"><input type="checkbox" id="checked_record" name="checked_record[]" value="'+btoa(row.id)+'" class="checkItem case"><label style="text-decoration: none"></label></div>';
                    }
                  },
                  ...
                  {data: 'third_sub_category_name',"orderable": true, "searchable":false},

                  {data: 'fourth_sub_category_name',"orderable": true, "searchable":false},
                 ...
                 
                  {
                    render : function(data, type, row, meta) 
                    {
                        return '<a class="btn btn-circle btn-success btn-outline show-tooltip editstyle" href="'+row.build_edit_href+'" title="Edit">Edit</a>&nbsp;<a class="btn btn-outline btn-info btn-circle show-tooltip deletestyle" href="'+row.built_delete_href+'" onclick="confirm_delete(this,event);" title="Delete">Delete</a>';
                    },
                    "orderable": false, "searchable":false
                  }]
        });
     ...
     /*   $('#checked_record_all').change(function () 
        ...
        $('#table_module').on('draw.dt',function(event)
        {
         ...
         $("#table_module").find("thead").append(`<tr>
                    ...
                    <td><input type="text" name="q_third_sub_category" placeholder="Search" class="search-block-new-table column_filter form-control" /></td>
                    <td><input type="text" name="q_fourth_sub_category" placeholder="Search" class="search-block-new-table column_filter form-control" /></td>
    ...
    function change_status(reference)
    {
        ...
        if(action == 'activate')
        {
           var msg = 'Are you sure? Do you want to activate this fourth level category.';
        }
        else if(action == 'deactivate')
        {
          var msg = 'Are you sure? Do you want to deactivate this fourth level category.'; 
        }
                    ...
                    $.ajax({
                        ...
                        { 
                            ...
                            { 
                                ...
                                {
                                  swal("warning", data.message, data.status);
                                }
                                else
                                { 
    ...
    {
      confirm_action(ref,event,'Are you sure? Do you want to delete this fourth level category.');
    }
