            ...
            <div class="col-sm-12 col-xs-12">
               ...
               <div class="row m-b-20">
                  ...
                  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10">
                     <input type="text" class="form-control" name="area_name" placeholder="Enter Area Name" data-parsley-required="true"  data-parsley-required-message="Please enter area name" value="{{$rep_area['area_name']}}" />
               ...
               <div class="row m-b-20">
                   <label class="col-xs-12 col-sm-12 col-md-12 col-lg-2 col-form-label">States Covered<i class="red">*</i></label>
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10">
                      ...
                      <select id="boot-multiselect-demo" name="rep_state[]" multiple="multiple" data-parsley-required="true" data-parsley-required-message="Please select states covered" data-parsley-errors-container="#error_container">
                       @if(isset($state_names) && count($state_names)>0)
                               @foreach($state_names as $key=>$state)
                                  {{-- @if(isset($state['user_details']) && count($vendor['user_details'] > 0)) --}}

                                     <option value="{{$state['id']}}" {{(in_array($state['id'], $state_arr)) ? 'selected' : '' }}>{{$state['name']}}</option>
                      ...
                      <span id="error_container" class="red">{{ $errors->first('rep_state') }}</span> 
                    </div>
                ...
                <div class="row m-b-20">
                   <label class="col-xs-12 col-sm-12 col-md-12 col-lg-2 col-form-label">Division Categories</label>
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10">
                      ...
                      <select id="boot-multiselect-demo_category" name="category_names[]" multiple="multiple">
                          
                          @if(isset($category_names) && count($category_names)>0)
                       
                               @foreach($category_names as $key=>$category)

                                    @if(!empty($category_arr) && in_array($category['id'], $category_arr) != "")
                                     <option value="{{$category['id']}}" {{(in_array($category['id'], $category_arr)) ? 'selected' : '' }}>{{$category['cat_division_name']}}</option>
                                    @else
               ...
               <div class="row m-b-20">
                    <label class="col-xs-12 col-sm-12 col-md-12 col-lg-2 col-form-label">Status</label>
                      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10">
                              @php
                                if(isset($rep_area['status'])&& $rep_area['status']!='')
                                {
                                  $status = $rep_area['status'];
                                } 
                                else
                                {
                                  $status = '';
                              ...
                              @endphp
                              <input type="checkbox" name="area_status" id="area_status" value="1" data-size="small" class="js-switch " @if($status =='1') checked="checked" @endif data-color="#99d683" data-secondary-color="#f96262" onchange="return toggle_status();" />
                          </div>   
                  </div>
                    <div class="cancel_back_flex_btn ">
                      ...
                      <button class="btn btn-success waves-effect waves-light" type="button" name="Save" id="btn_update" value="Update">Save</button>
                  </div>
               </form>
            </div>
         </div>
       ...
       $('#btn_update').click(function(){
        ...
        $.ajax({
          ...
          success:function(data)
          {
            ...
            msg_status = data.status.charAt(0).toUpperCase()+data.status.slice(1);
