                    ...
                    <form class="form-horizontal" id="frm_manage" method="POST" action="{{ url($module_url_path.'/report_generator') }}">
                        ...
                        <div class="row">
                          <!-- <div class="col-sm-4">
                            ...
                            <div class="col-sm-12 commission-below-report-btns text-right mb-0">
                             <div class="dataTables_length" id="table_module_length">
                                ...
                                <button type="button" id="table_module_length" title="Click for Representative Bulk Payment" class="btn btn-success waves-effect waves-light filter_btn pull-right mr-2"  onclick="bulkPayment_validation('representative');">
                                      <i class="fa fa-paper-plane" aria-hidden="true"></i>  
                                      Bulk Pay Representative
                                </button>

                                <button type="button" id="table_module_length" title="Click for Sales Manager Bulk Payment" class="btn btn-success waves-effect waves-light filter_btn pull-right mr-2"  onclick="bulkPayment_validation('sales_manager');">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="vendorPaymentForm"> 
          ...
          <input type="hidden" name="order_flag" id="order_flag">
...
<script type="text/javascript">
  ...
  $(document).ready(function()
  {
    ...
    $('#update').click(function(){
     ...
     $('.datepicker').datepicker({
      ...
      {data: 'commission_status', "orderable": false, "searchable":false},

      {data: 'rep_commission_amount', "orderable": false, "searchable":false},
    ...
    
      {
        render : function(data, type, row, meta) 
        {
          ...
          console.log("total_rep_commission",total_rep_commission);
          console.log("rep_commission_paid",rep_commission_paid);
          console.log("total_rep_commission",admin_earned_commission);
  ...
  // function fillData(orderPrice,vendorAmount,adminCommissionAmount,makerId,orderId,orderNo = false)

function fillData(adminCommissionPercent,orderPrice,vendorAmount,adminCommissionAmount,makerId,orderId,orderNo= false,$orderFlag=false)
{
  $('.vendor-Modal').modal('show');
  $('#order_amount').html(orderPrice.toFixed(2));    
  $('#actual_amount').html(adminCommissionAmount.toFixed(2));
  $('#admin_commission_per').html(adminCommissionPercent.toFixed(2));  
  $('.pay_amount').html(vendorAmount.toFixed(2));    
  $('#maker_id').val(makerId);    
  $('#amount').val(vendorAmount.toFixed(2));    
  $('#orderId').val(orderId); 
  $('#orderNo').val(orderNo); 
  $("#order_flag").val($orderFlag);      
}
...
function paySalesCommission(orderPrice,adminCommission,repcommission,repCommissionAmount,salesId,orderId,$orderFlag=false)
{
  $('.rep-Modal').modal('show');
  $('#rep_commission').html(repcommission);    
  $('#rep_pay_amount').html(repCommissionAmount.toFixed(2));    
...
function payVendor()
{
  ...
  // var paymentFormData = $("#vendorPaymentForm").serialize();
...
{ 
  ...
  if(order_flag == 'rep_sales_order')
  {
     var url = '{{url('/admin/leads')}}'+'/pay_commission';
  } 
  else if(order_flag == "retailer_orders")
  {
     var url = '{{url('/admin')}}'+'/payment/vendor';
  }
  else
  {
     var url = '';
...
function load_BulkPayment_model(checkOrderItems, id, user_type)
{
    ...
    if(user_type == 'vendor'){
    //var generated_url  =  "{{url('load_bulkPaymentModelData_cust_vendor')}}";
    ...
    $.ajax({
               ...
               type:"POST",
               dataType:'html',
               data:{'_token':csrf_token, 'checkOrderItems': checkOrderItems, 'user_id': id, 'user_type': user_type, 'order_type': 'retailer'},
               beforeSend: function() 
               {
