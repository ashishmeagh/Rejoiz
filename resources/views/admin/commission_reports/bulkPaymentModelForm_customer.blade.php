...
<?php 
    ...
    switch($user_type)
    {
        ...
        case "vendor": $col1 = "Vendor"; $col1_key = "vendor_name";
                               $col2 = "vendor"; $col2_key = "vendor_name";  
                               $commission_to = "Vendor";
                               $commission_forKey = "admin_name";
                             break;
...
<form id="bulkPay_modelForm" name="bulkPay_modelForm" method="post">
    ...
    <table id="example" class="table table-striped table-bordered" style="width:100% font-size: 12px;">
        <thead>
            <tr>
                ...
                <th class="modelTbl_th">{{ ($order_type == 'customer') ? 'Customer' : 'Retailer' }}</th>
                ...
                <!-- <th class="modelTbl_th">{{$commission_to}} <br/> Commission</th> -->
                <th class="modelTbl_th">Amount Payable to Vendor</th>
        ...
        @php 
            ...
            { 
                ...
                $commission_for = isset($user_details[$order_id]['vendor_name']) ? $user_details[$order_id]['vendor_name'] : "--";

                foreach($order_rows as $rowArr)
                {
                    ...
                    $total_repCommissionAmount += $rowArr['vendorCommissionAmount'];
           ...
           @endphp
            ...
            <tr id="model_tr_{{$rowArr['order_id']}}" style="background-color:{{$bg_color}}">
                ...
                <input type="hidden" name="order_noArr[{{$rowArr['order_id']}}]" value="{{$user_details[$order_id]['order_no']}}">
                <input type="hidden" name="adminCommission[{{$rowArr['order_id']}}]" value="{{$rowArr['adminCommission']}}">
                <input type="hidden" name="orderPrice[{{$rowArr['order_id']}}]" value="{{$rowArr['orderPrice']}}">
                 <input type="hidden" name="repCommissionAmount[{{$rowArr['order_id']}}]" value="{{$rowArr['vendorCommissionAmount']}}">
              <!--  -->
                ...
                <td align="center"> 
                    <?php
                        echo $user_details[$order_id]['order_no'];
                    ?>  
                </td>
                ...
                <td align="center">
                    <?php
                        if(isset($user_details[$order_id]['customer_name']))
                            echo $user_details[$order_id]['customer_name'];
                        else
                            echo "--";
                    ?>  
                </td>
                ...
                <td align="center"><?php
                        if(isset($user_details[$order_id]['admin_commission']))
                            echo num_format($user_details[$order_id]['admin_commission']);
                        else
                            echo "--";
                    ?>  %</td>
                <td align="center">
                ...
                <td align="center">
                    $<span id="repCommissionAmount_{{$rowArr['order_id']}}">
                       {{num_format($rowArr['vendorCommissionAmount'])}}
                    </span>
                </td>
                ...
                <td align="center">
                   ...
                   <span class="loader" id="loader_deleteRow_{{$rowArr['order_id']}}" style="color: #07892f; text-align:center;display: none;"> <i class="fa fa-spinner fa-pulse fa-3x fa-fw" style="font-size: 17px;"></i> </span>
                </td>
             </tr>
    ...
    <input type="hidden" name="total_vendorCommissionAmount" id="total_vendorCommissionAmount" value="{{$total_repCommissionAmount}}">
</form>
...
<script type="text/javascript">
    ...
    function cal_total(order_id, is_checkBox=0)
    {
        ...
        {
            // alert(repCommissionAmount_total);
            // alert(repCommissionAmount);
            var cal_orderPrice_total = parseFloat(orderPrice_total) - parseFloat(orderPrice);
            var cal_adminCommission_total = parseFloat(adminCommission_total) - parseFloat(adminCommission);
            var cal_repCommissionAmount_total = parseFloat(repCommissionAmount_total) - parseFloat(repCommissionAmount);

        }
        // alert(cal_orderPrice_total);
        // alert(cal_adminCommission_total);
        // alert(cal_repCommissionAmount_total);
