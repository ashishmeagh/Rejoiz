                    ...
                    @if($lang['locale'] == 'en')
                      ...
                      <div class="form-group row">
                        <label class="col-xs-12 col-sm-12 col-md-12 col-lg-2 col-form-label">Priority<i class="red">*</i></label>
                          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10 controls">
                              @php
                                if(isset($arr_data['priority'])&& $arr_data['priority']!='')
                                {
                                  $priority = $arr_data['priority'];
                                } 
                                else if($arr_data['priority'] == null)
                                {
                                  $priority = 0;
                                }
    
                              @endphp
                              <input type="text" name="priority" id="priority" class="form-control" value="{{$priority}}" placeholder="Enter the priority" data-parsley-pattern="^[0-9*#+]+$" data-parsley-required data-parsley-required-message="Please enter the priority." data-parsley-pattern-message="Only numbers are allowed." />
                          </div>
              ...
              @endif
               ...
               <div class="form-group row">
                      ...
                      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-10 controls">
                        @php                
                           $product_img_path = "";
                           $image_name = (isset($arr_data['category_image']))? $arr_data['category_image']:"";
                           $image_type = "category";
                           $is_resize = 0;
                           $product_img_path = imagePath($image_name, $image_type, $is_resize); 
                        @endphp                 
                        <input type="file" name="category_image" id="category_img" class="dropify" data-default-file="{{ $product_img_path }}" data-allowed-file-extensions="png jpg JPG jpeg JPEG" data-max-file-size="2M" data-errors-position="outside" 
                        />
                      </div>
                    </div>
