...
<table width="100%" bgcolor="#fff" border="0" cellspacing="0" cellpadding="0" style="border:0px solid #ddd;font-family:Arial, Helvetica, sans-serif;">
            ...
            <table width="100%">
             <tbody>
                ...
                <div class = "data_container">
                    ...
                    {{isset($customer_data['user_details']['first_name'])?$customer_data['user_details']['first_name'].' ':''}}{{isset($customer_data['user_details']['last_name'])?$customer_data['user_details']['last_name'].',':''}}<br>

                    {{isset($customer_data['user_details']['customer_details']['address2'])?$customer_data['user_details']['customer_details']['address2'].', ':''}}{{isset($customer_data['user_details']['address'])?$customer_data['user_details']['address'].',':''}} <br>


                    {{isset($customer_data['user_details']['customer_details']['city'])?$customer_data['user_details']['customer_details']['city'].', ':''}}{{isset($customer_data['user_details']['customer_details']['state'])?$customer_data['user_details']['customer_details']['state'].',':''}} <br>

                    {{isset($customer_data['user_details']['country_id'])?get_country($customer_data['user_details']['country_id']).', ':''}}{{isset($customer_data['user_details']['post_code'])?$customer_data['user_details']['post_code']:''}}
        ...
        <td colspan="2">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td style="border-bottom: 1px solid #e5e9f1;font-size:12px;text-align: left;padding:12px 12px 12px 30px;">${{isset($product_final_total)?num_format($product_final_total):0}} <?php $sub_total += $product_final_total; ?></td>
                <?php 
    ...
    if (isset($customer_data['promotion_discount'])) {
        $promo_discount = $customer_data['promotion_discount'];
    ...
    @if(isset($customer_data['promotion_discount']))
        ...
        <tr>
            <td width="80%" style="background-color: #d5d8dc;font-size:13px;font-weight: bold;text-align: right;border-bottom:1px solid #d5d8dc;padding:12px;">
                Promotion Discount (@if($customer_data['discount_percent']>0)@endif{{isset($customer_data['discount_percent'])?num_format($customer_data['discount_percent']):0}}%)
            </td>
            ...
            <td width="20%" style="background-color: #d5d8dc;font-size:13px;font-weight: bold;text-align: right;border-bottom:1px solid #d5d8dc;padding:12px;">
                @if($customer_data['promotion_discount']>0)-@endif${{isset($customer_data['promotion_discount'])?num_format($customer_data['promotion_discount']):0}}
            </td>
        </tr>
