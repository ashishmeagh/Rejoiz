               ...
               <table width="100%">
                  <tr>
                     ...
                     <td width="50%" style="background-color: #d3d7de;padding:10px;font-size:12px; text-align: right;">
                        @if(isset($order_date))
                        <b>Order Date:</b> {{isset($order_date)?us_date_format($order_date):''}}
                        @else
                  ...
                  <tbody>
                     <tr>
                        <td width="30%" style="font-size:12px; text-align: left; word-break: break-all;">
                           <h3 style="margin-bottom: 5px;">Order From (Customer{{-- {{isset($Role)?$Role:'Customer'}} --}}) :</h3>
                           @if(isset($Role) && $Role == 'Retailer')
                           @php
                           ...
                           <div class = "data_container">                
                              ...
                              <br>
                              @endif
                              {{ rtrim($country,', ')}} {{$post_code or ''}} 
                              {{-- <b>{{isset($retailer_data['store_name'])?$retailer_data['store_name']:''}}</b><br>
                              <b>{{isset($retailer_data['store_website'])?$retailer_data['store_website']:''}}</b><br>
                           ...
                           @elseif(isset($Role) && $Role == 'Representative')
                           @php
                        ...
                        <td  width="40%">&nbsp; </td>
                        <td width="30%" style="font-size:12px; text-align: right; word-break: break-all;">
                           <h3 style="margin-bottom: 5px;">Order To (Vendor) :</h3>
                           <div class= "data_container2">
                              <b>{{$maker_addr['company_name'] or 'N/A'}}</b><br>
                              @if(@$hide_vendor_address_from_retailer != '1')
                                 {{isset($maker_addr)?$maker_addr['address']:'N/A'}}

                                 @if(isset($maker_addr['country_id']) && isset($maker_addr['address']) && $maker_addr['address'] != "")
                                    <br>
                                 @endif                              
                                    {{isset($maker_addr['country_id'])? get_country($maker_addr['country_id']):'N/A'}}
                     ...
                     <tr>
                        <td colspan="3">
                           <table width="100%">
                              ...
                              @endif
                              @if(isset($retailer_data['email_type']) && ($retailer_data['email_type'] == 'order_cancel_by_retailer' || $retailer_data['email_type'] == 'order_cancel_by_vendor'))
                              <td width="50%" style="font-size:12px;">
            ...
            <td colspan="2">
               <table width="100%" border="0" cellpadding="0" cellspacing="0">
                  ...
                  <?php
                     ...
                     {
                         $product_discount_amount = isset($ord['product_discount_amount'])?num_format($ord['product_discount_amount']):0;
                     ...
                     if(isset($ord['product_discount']))
                     {
                         ...
                         {
                             $product_discount_amount = '-'.(isset($ord['product_discount'])?num_format($ord['product_discount']):0);
                  ...
                  <tr>
                     ...
                     <td style="border-bottom: 1px solid #e5e9f1;font-size:12px;text-align: left;padding:12px 12px 12px 30px;"><b>{{$ord['product_name'] or '-'}}|{{ $ord['color'] or '-'}}|
                     @if(isset($ord['size_id']) && $ord['size_id'] != "")   
                     {{ get_size_from_id($ord['size_id'])}}
                     @endif
                     </td>
